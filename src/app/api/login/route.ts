...
export async function POST(request: Request) {
  ...
  if (userService != undefined && userService.password == createHash('SHA3-512').update(`${userService.saltA}${password}${userService.saltB}`).digest('hex')) {
    // @ts-ignore
    token = await new jwt.SignJWT({id: username}).setProtectedHeader({alg: 'HS256'}).setAudience('Loom').setExpirationTime('10 d').sign(createSecretKey('avnceStagingDeveloperInIT', 'utf-8'))
    authenticated = true    
  } else {
    token = undefined;
    authenticated = false;
